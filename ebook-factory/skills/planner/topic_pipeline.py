...
# ── Ollama (via shared ollama_client with retry) ─────────────────────────────────

# Import shared Ollama client
sys.path.insert(0, str(HERMES_HOME / "ebook-factory" / "skills"))
...
def ollama(prompt: str, system: str, num_predict: int = 3000,
    ...
    """Call Qwen 27B with retry. Raises on connection failure after all retries."""
    result = ollama_call_with_retry(
        prompt, system, PLANNER_MODEL,
        max_retries=3,
    ...
    if result is None:
        die(f"Ollama call failed after 3 retries for model {PLANNER_MODEL}")
