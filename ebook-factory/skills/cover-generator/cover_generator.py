...
def build_background_prompt(palette: dict, meta: dict, book_dir: Path | None = None) -> str:
    ...
    if book_dir is not None:
        custom_path = book_dir / "cover_prompt.txt"
        if custom_path.exists():
            custom = custom_path.read_text(encoding="utf-8").strip()
            if len(custom) >= 50:
                log(f"  Using book-specific cover prompt from {custom_path.name}")
                return (
...
def generate_cover(meta: dict, book_dir: Path,
    ...
    if dry_run:
        ...
        custom_exists = book_dir and (book_dir / "cover_prompt.txt").exists()
        log(f"  [DRY RUN] Prompt source: {'cover_prompt.txt (book-specific)' if custom_exists else 'niche palette'}")
        log(f"  [DRY RUN] BG prompt snippet: {build_background_prompt(palette, meta, book_dir)[:80]}...")
        log(f"  [DRY RUN] Cost estimate: ${0.03 + (0.06 if upscale else 0):.2f}")
        return None

    # Step 1: Generate background
    bg_prompt = build_background_prompt(palette, meta, book_dir)
